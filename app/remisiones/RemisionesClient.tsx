...
interface RemisionesClientProps {
  remisionesIniciales: VentaLista[];
}

export default function RemisionesClient({ remisionesIniciales }: RemisionesClientProps) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();

  const [busqueda, setBusqueda] = useState("");
  const [filtroEstado, setFiltroEstado] = useState<string>("TODOS");
  const [remisionSeleccionada, setRemisionSeleccionada] = useState<VentaLista | null>(null);
  ...
  const remisionesFiltradas = useMemo(() => {
    return remisionesIniciales.filter((r) => {
      const codigo = `R-${String(r.consecutivo).padStart(4, "0")}`;
      const matchBusqueda =
        !busqueda ||
  ...
  }, [remisionesIniciales, busqueda, filtroEstado]);
  ...
  async function handleConvertir() {
    ...
    const pagos = [
      {
        metodo: metodoPago,
        monto: remisionSeleccionada.total,
        referencia: referenciaPago || undefined,
      },
    ];

    const res = await convertirDocumento(remisionSeleccionada.id, "VENTA", pagos);
  ...
  const columns = [
    {
      ...
      render: (r: VentaRow) => (
        <span className="font-mono font-bold text-slate-900">
          R-{String(r.consecutivo).padStart(4, "0")}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (r: VentaRow) => (
        <span className="font-medium text-slate-800">
          {r.cliente?.nombre ?? "Cliente General"}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (r: VentaRow) => (
        <span className="text-xs font-semibold bg-slate-100 text-slate-700 px-2 py-0.5 rounded-full">
          {r.items.reduce((a, i) => a + i.cantidad, 0)} arts
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (r: VentaRow) => {
        if (r.estado === "COMPLETADA") {
          return <Badge variant="success">Facturada / Venta</Badge>;
        }
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeader
        title="Remisiones"
        description="Registro y conversión de salidas de mercancía en consignación o pendientes de pago."
        actions={
          <Link href="/ventas/nueva?tipo=REMISION">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        <div className="relative w-full sm:w-80">
          ...
          <Input
            ...
            placeholder="Buscar por N° Remisión, cliente…"
            className="pl-9"
          />
        </div>
          ...
          <div className="w-44">
            <Select value={filtroEstado} onValueChange={setFiltroEstado}>
              ...
              <SelectContent>
                ...
                <SelectItem value="COMPLETADA">Facturadas</SelectItem>
      ...
      <DataTable
        ...
        pageTitle="Remisiones Registradas"
      />
      ...
      <Dialog open={modalDetalleOpen} onOpenChange={setModalDetalleOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              Detalle de Remisión R-{String(remisionSeleccionada?.consecutivo ?? 0).padStart(4, "0")}
            </DialogTitle>
            <DialogDescription>
              Creada el {remisionSeleccionada ? new Date(remisionSeleccionada.createdAt).toLocaleString("es-CO") : ""}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4 text-xs">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-slate-50 p-3 rounded-lg">
                <div>
                  ...
                  <span className="font-semibold text-slate-800">
                    {remisionSeleccionada.cliente?.nombre ?? "Cliente General"}
                  </span>
                </div>
                <div>
                  <span className="text-slate-500 block">Vendedor:</span>
                  <span className="font-semibold text-slate-800">
                    {remisionSeleccionada.vendedor?.nombre ?? "Sin asignar"}
                  </span>
                </div>
                <div>
                  <span className="text-slate-500 block">Bodega:</span>
                  <span className="font-semibold text-slate-800">
                    {remisionSeleccionada.bodega.nombre}
                  </span>
                </div>
              </div>
              ...
              <div>
                <h4 className="font-bold text-slate-700 uppercase tracking-wider mb-2">
                  Ítems Remisionados ({remisionSeleccionada.items.length})
                </h4>
              ...
              <div className="flex justify-between items-center bg-slate-100 p-3 rounded-lg font-bold text-sm text-slate-900">
                ...
                <span>{formatoCOP(remisionSeleccionada.total)}</span>
              </div>
            </div>
          )}
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-emerald-700">
              <ArrowRightLeft className="h-5 w-5" /> Convertir Remisión a Venta
            </DialogTitle>
            <DialogDescription>
              Se formalizará el cobro de la remisión R-
              {String(remisionSeleccionada?.consecutivo ?? 0).padStart(4, "0")}, generando una nueva factura de venta y registrando el pago en caja.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4">
              <div className="bg-emerald-50 border border-emerald-200 text-emerald-900 p-3 rounded-lg text-xs space-y-1">
                ...
                <p className="text-[11px] text-emerald-700">
                  El inventario ya fue descontado previamente al emitir la remisión.
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Ej. N° aprobación tarjeta o boucher..."
                  />
                </div>
          ...
          <DialogFooter className="gap-2">
            ...
            >
              {pending ? "Procesando…" : "Confirmar Cobro y Facturar"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-red-600 flex items-center gap-2">
              <AlertCircle className="h-5 w-5" /> Anular Remisión R-
              {String(remisionSeleccionada?.consecutivo ?? 0).padStart(4, "0")}
            </DialogTitle>
            <DialogDescription>
              Al anular la remisión se devolverán los artículos al inventario de la bodega.
            </DialogDescription>
          </DialogHeader>
